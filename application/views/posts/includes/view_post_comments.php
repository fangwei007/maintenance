<section class="comment-block">
    ...
    <div class="comment-area">
        <form id="submit-comment-form" method="POST" action="<?= base_url() . "comments/add_comment/" . $post->PostID . "/post"?>">
...
<script type="text/javascript">
    ajax_load_view('.comment-content', '/comments/show_comments/<?= $post->PostID?>/post');
    //set comments scrolling load
    $(window).scroll(function(){
        if($(window).scrollTop() === $(document).height() - $(window).height() && $(".comment-cell").length > 0){
            $('div#load-more').show();
            var pathname = "<?= base_url() ?>comments/load_more_comments/<?= $post->PostID?>/post/" + $(".comment-cell").length;
         $.ajax({
             url: pathname,
